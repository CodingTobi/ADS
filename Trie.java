...
public class Trie {
	static class TrieNode {
		...
		/* Fügt den Buchstaben c in den Suchbaum des TrieNodes ein. */ public void add(char c, int count) {
			st.inc(c, count);
		...
		/* Liefert den zu c gehörigen TrieNode zurück */ public TrieNode get(char c) {
			return st.getNode(c);
		}
	}
	...
	public static class SearchTree {
		...
		/* Liefert den Zähler des Schlüssels key zurück */ public long getCount(char key) {
			if (root == null)
				return 0L;
		...
		/* Liefert das (TrieNode-)Kind für key zurück */ public TrieNode getNode(char key) {
			if (root == null)
				return null;
			Node current = root;
	...
	/* Fügt einen Trainingsquery zum Trie hinzu */ public void add(String s, int count) {
